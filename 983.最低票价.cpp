...
public: 
    ...
    void backtrace(vector<int>& days, vector<int>& costs, vector<int>& mem, int spend, int index) {
        ...
        if (mem[index] == 0 || (mem[index] != 0 && spend < mem[index]) )
            mem[index] = spend;
        else return;
        for (int i = 0; i < costs.size(); i++) {
            spend += costs[i];
            if (spend > ans) return;
            int deadline = days[index] + duration[i];
            int tmp = 0;
            while (index + tmp < length && days[index + tmp] < deadline) tmp += 1;
            backtrace(days, costs, mem, spend, index + tmp);
            spend -= costs[i];
        }
    }
    void dp(vector<int>& days, vector<int>& costs, vector<int>& mem) {
        for (int i = 1; i < days.back()+1; i++) {
            if (find(days.begin(), days.end(), i) != days.end()) {
                int tmp;
                tmp = min(mem[i-1] + costs[0], mem[i-7<0 ? 0 : i-7] + costs[1]);
                tmp = min(tmp, mem[i-30<0 ? 0 : i-30] + costs[2]);
                mem[i] = tmp;
            } else {
                mem[i] = mem[i-1];
            }
